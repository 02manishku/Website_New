        ...
        <div className="mx-auto max-w-[1400px] px-6 lg:px-10 grid md:grid-cols-2 gap-2">
          {CATALOGS.map((c) => (
            <a
              key={c.name}
              href="#"
              className="group flex items-center justify-between gap-8 p-10 bg-sandlight border hairline hover:bg-sand transition-colors"
            >
              <div>
                <div className="kicker text-smoke mb-2">{c.cover}</div>
                <h3 className="font-display text-3xl text-ink leading-tight">{c.name}</h3>
                <div className="kicker text-smoke mt-3">{c.size}</div>
              </div>
              <div className="kicker text-ink whitespace-nowrap group-hover:translate-x-2 transition-transform">
                Download →
              </div>
            </a>
          ))}
